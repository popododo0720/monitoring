...
func InsertProcessCPUData(db *sql.DB, startTime, endTime time.Time) error {
	...
	for _, item := range data["result"].([]interface{}) {
		...
		for _, v := range valueData {
			...
			_, err = db.Exec(
				"INSERT INTO instance_process_cpu(command, pid, instance_user, instance, timestamp, cpu_usage) VALUES ($1, $2, $3, $4, $5, $6)",
				command,
				pid,
				user,
				instance,
...
func InsertProcessMEMData(db *sql.DB, startTime, endTime time.Time) error {
	...
	for _, item := range data["result"].([]interface{}) {
		...
		for _, v := range valueData {
			...
			_, err = db.Exec(
				"INSERT INTO instance_process_mem(command, pid, instance_user, instance, timestamp, mem_usage) VALUES ($1, $2, $3, $4, $5, $6)",
				command,
				pid,
				user,
				instance,
